...
export default function EbookAdminPage() {
  ...
  const handleInputChange = (e) => {
    ...
    editingEbook ? setEditingEbook({ ...editingEbook, [name]: value }) : setNewEbook({ ...newEbook, [name]: value });
  };

  const handleCategoryChange = (e) => {
    const selected = [e.target.value];
    editingEbook ? setEditingEbook({ ...editingEbook, categories: selected }) : setNewEbook({ ...newEbook, categories: selected });
  };
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      editingEbook ? setEditingEbook({ ...editingEbook, ...update }) : setNewEbook({ ...newEbook, ...update });
    } catch (err) {
      console.error(err);
      ...
      {filtered.map((book) => (
        <div key={book.id} className="border p-4 rounded mb-4 flex justify-between items-start">
          <div>
            ...
            <p className="text-sm text-gray-500">Formátum: {book.file_format} | Méret: {book.file_size_mb} MB</p>
